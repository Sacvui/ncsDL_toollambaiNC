...
Khoảng cách 3 tuần giữa hai đợt đủ dài để phá vỡ hiệu ứng nhất quán cảm xúc (consistency motif) nhưng đủ ngắn để giảm thiểu tỷ lệ hao hụt mẫu (attrition rate). Dựa trên tỷ lệ phản hồi thận trọng 30-40% trong nghiên cứu hành vi tiêu dùng thực tế, cần phát ra tối thiểu 2.500-3.000 bảng hỏi ở Wave 1 để thu về ít nhất 900 cặp dữ liệu hợp lệ. Nghiên cứu sẽ sử dụng hệ thống incentive (phiếu mua hàng/voucher trị giá 20.000-50.000 VND) và cơ chế nhắc nhở qua SMS/Zalo sau 1 tuần và 2 tuần để tối đa hóa tỷ lệ giữ chân mẫu.

**Kiểm định sự khác biệt nhóm (Non-response Bias Test):** Sau khi thu thập xong Wave 2, thực hiện kiểm định Independent Samples t-test so sánh các đặc điểm nhân khẩu học và điểm trung bình các biến Wave 1 giữa hai nhóm: (1) những người phản hồi cả Wave 2, và (2) những người chỉ phản hồi Wave 1 rồi bỏ cuộc. Nếu không có sự khác biệt có ý nghĩa thống kê (p > 0.05) trên các biến chính, kết luận việc hao hụt mẫu mang tính ngẫu nhiên và không tạo ra thiên lệch hệ thống (systematic bias).
...
Sau khi tiếp xúc với kịch bản, cả hai nhóm được đo lường: (1) Tính nhạy cảm về giá (Price Sensitivity - PS) như biến trung gian tâm lý giải thích cơ chế "tại sao" thuật toán bào mòn lòng trung thành; (2) Lòng trung thành (CL), Ý định chi trả giá cao (WTPP), và Tỷ trọng chi tiêu (eSOW) như biến kết quả. Study 2 nhằm chứng minh quan hệ nhân quả (causal inference) một cách tuyệt đối cho giả thuyết: thuật toán AI gợi ý sản phẩm thay thế làm tăng tính nhạy cảm về giá (Price Sensitivity), thông qua đó làm giảm lòng trung thành và WTPP. Thiết kế Mediation-within-Experiment này bổ khuyết cho Study 1 vốn chỉ xác lập quan hệ tương quan, đồng thời giải thích rõ cơ chế tâm lý trung gian.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 920 480" width="100%" font-family="Arial,sans-serif" font-size="12">
  ...
  <text x="770" y="128" text-anchor="middle" font-weight="bold" font-size="11">CPO1</text>
  <text x="770" y="145" text-anchor="middle" font-size="9">WTPP (Premium)</text>
  <ellipse cx="770" cy="325" rx="72" ry="26" fill="#fff" stroke="#000" stroke-width="1.5"/>
  <text x="770" y="318" text-anchor="middle" font-weight="bold" font-size="11">CPO2</text>
  ...
  <text x="630" y="317" font-size="10" font-weight="bold">H9</text>
  ...
  <rect x="470" y="420" width="160" height="38" rx="3" fill="#fafafa" stroke="#777" stroke-width="1" stroke-dasharray="4,3"/>
  <text x="550" y="436" text-anchor="middle" font-size="10" font-weight="bold">W2: Quy mô TH</text>
  <text x="550" y="451" text-anchor="middle" font-size="9" fill="#555">Nhỏ / Vừa / Lớn</text>
  ...
  <!-- H11: Brand Size moderates CSI->CL path. Midpoint ~(415, 174) -->
  <line x1="550" y1="420" x2="418" y2="180" stroke="#777" stroke-width="1.2" stroke-dasharray="5,3" marker-end="url(#ag)"/>
  <text x="490" y="305" font-size="9" fill="#555">H11</text>
...
**Chú giải:**
- Hình Elip: biến tiềm ẩn (Latent constructs) đo lường bằng nhiều items
...
- H11: Quy mô thương hiệu điều tiết đường dẫn CSI -> CL
...
**Chú giải Hình 2:**
- Đường a: Tác động của thuật toán AI lên tính nhạy cảm về giá (Price Sensitivity)
...
| CPO2 | Tỷ trọng chi tiêu dự kiến (eSOW) | Biến kết quả | Frequency Allocation (/10) | Keiningham et al. (2007), điều chỉnh | Wave 2 |
...
**CPO2: Tỷ trọng chi tiêu dự kiến (eSOW)** *(Thang đo phân bổ tần suất - Frequency Allocation Scale, điều chỉnh từ Keiningham et al., 2007)* — Wave 2

| Mã | Nội dung câu hỏi |
|---|---|
| eSOW | Giả sử trong **10 lần mua** sản phẩm [danh mục] tiếp theo, bạn dự kiến sẽ mua [TH] bao nhiêu lần? Mua thương hiệu khác bao nhiêu lần? *(Ví dụ: [TH] = ___ lần, Thương hiệu B = ___ lần, Thương hiệu C = ___ lần, Thương hiệu khác = ___ lần. Tổng = 10 lần)* |

*Ghi chú: eSOW được đo bằng phân bổ tần suất (Frequency Allocation) thay vì phân bổ tỷ lệ % ngân sách. Cách hỏi "số lần mua trong 10 lần tiếp theo" giúp giảm rào cản nhận thức (cognitive load) đáng kể cho người tiêu dùng FMCG vốn mua sản phẩm giá trị nhỏ, so với việc bắt họ tính nhẩm % ngân sách. Tỷ trọng dành cho [TH] (ví dụ: 7/10 = 70%) sẽ được quy đổi thành biến liên tục tỷ lệ (ratio continuous variable) làm giá trị đầu vào cho mô hình PLS-SEM.*
...
*Ghi chú: Thang đo ARD được phát triển mới theo quy trình chuẩn (DeVellis & Thorpe, 2022): (1) Tạo item pool từ phỏng vấn định tính, (2) Đánh giá nội dung bởi panel chuyên gia (Content Validity Index), (3) Kiểm định sơ bộ qua Pilot Study (n=80), (4) Tinh chỉnh và xác nhận trong Study 1. Để đảm bảo giá trị mạng lưới khái niệm (Nomological Validity), kiểm tra rằng ARD có tương quan dương có ý nghĩa thống kê với thời gian sử dụng smartphone hàng ngày (r > 0.3, p < 0.01) và tương quan âm với độ tuổi (người trẻ phụ thuộc thuật toán nhiều hơn). Nếu các tương quan này không đạt, thang đo sẽ được điều chỉnh lại trước khi đưa vào khảo sát chính thức.*
...
**Biến trung gian tâm lý (Psychological Mediator):** Tính nhạy cảm về giá (Price Sensitivity - PS), đo lường bằng 3 items (Likert 5): (1) Sau khi xem kịch bản, tôi quan tâm nhiều hơn đến việc tìm mức giá thấp nhất; (2) Tôi cảm thấy mình đang trả quá nhiều cho thương hiệu quen thuộc; (3) Sự khác biệt giá giữa các thương hiệu trở nên quan trọng hơn với tôi. Kế thừa và điều chỉnh từ Lichtenstein et al. (1993).
...
**Phân tích:** (1) So sánh trung bình giữa hai nhóm bằng Independent Samples t-test cho tất cả biến kết quả. (2) Kiểm định trung gian trong thực nghiệm (Mediation-within-Experiment): sử dụng PROCESS macro Model 4 (Hayes, 2022) để kiểm tra chuỗi nhân quả Thuật toán AI -> Price Sensitivity -> CL/WTPP/eSOW. (3) Phân tích ANCOVA với các biến kiểm soát (giới tính, độ tuổi, tần suất mua sắm online).
`;

fs.writeFileSync('c:\\ncsDL\\Proposal_HAILP_TDT\\05_PhuongPhap.md', content05, 'utf8');
console.log('Done overhauling 05_PhuongPhap.md - Round 3!');
